...
- D0-A 的人员与测试运营条件已由 Owner 授权；当前主 agent `/root` 担任测试运营负责人，正在形成双轨远端基线并安排 A01–A07 七个独立测试席位。
- 当前远端实施分支为 `codex/gate1-react-web`；`prototype/` 尚不存在，正式 RC 尚未冻结，因此 A01–A07 只安排、不提前实例化。
