...
- RC9 修复尚未开始，当前权威入口为 `docs/exec-plans/active/2026-07-27-gate1a-rc9-remediation-plan.md`。RC9 明确是 RC8 失败后获准的“责任承诺模型”受控转向，与原有效编辑指标不可直接同比；计划同时修复粮食/维修实际库存不跨周结转、一次性化肥跨周重复生效、tick 1002 换周双真值、accepted 人物请求漏计，以及摘要卡直接给答案的问题。计划采用场景 `0.5.0`、`gate1-playtest-v2`、A38–A44 全新七样本 cohort 与 A45/A46 预留替补；RC8 证据与失败结论保持只读。
- RC9 C01 因 D02 重启同一诊断编号被拒绝；C02 的 D06–D10 均技术有效，但 W1/W2 人工 v2 中位数均为 `2`，低于冻结区间 `3–5`，因此以 `REJECTED_DIAGNOSTIC_THRESHOLD` 拒绝。C03 当前权威入口为 `docs/exec-plans/active/2026-07-27-gate1a-rc9-c03-recovery-plan.md`：从 RC9-08 精确产品源码基线重新实现 W1 预防容量与 W2 恢复资源两项正交管理取舍，分配 P05/P06 和 D11–D15；C01/C02 与 P01–P04、D01–D10 永久只读，不得生成 CMxx。
