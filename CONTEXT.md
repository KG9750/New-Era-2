...
- `M-A`–`M-C` 已完成且不进入正式分母；RC8 独立冻结复核证据已形成，#7 等待证据提交后关闭。
...
- Gate 1A 当前为 `READY_PENDING_ROSTER`：RC8 使用 A30–A36，按 `A30–A32 / A33–A35 / A36` 执行，替补从 A37 开始；七个席位尚未创建，正式 player agent 尚未启动。
