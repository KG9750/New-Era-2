...
- 人物规则 V0.1-r8 已通过 Codex 独立终审；首个固定种子生成内核和 50 人候选库属于 `TECHNICAL_SPIKE_BEFORE_A1`。v0.1.4 首轮代码复审为 `P0=0 / P1=2 / REVIEW_FAIL`；其发现的零时长 work 刷技能与开放式 validation 字段已逐项修复：1–3 个工作节点逐节点从模板复算，技能经历受最低持续时间约束，library 根与权威 envelope 使用闭合 schema，308 条 `validation.findings` 的完整集合、顺序、metadata、结果与 warning 均由人物数组独立重建；当前生成器自重放仅保留为非权威 diagnostics。受审提交 `6c3b9ba1ed76353ce63b88c2c37277808369c683` 已通过第二轮 Codex 独立复审：`P0=0 / P1=0 / P2=0 / P3=0 / REVIEW_PASS`。在此基础上已生成 12 人 A1 前置样板包。固定提交 `1360dd1` 的全新 Codex 独立深刺结论为 `P0=0 / P1=4 / P2=3 / P3=0 / REVIEW_FAIL`：validator 关键值可绕过、阅读版泄露内部 MBTI、E/I 与技能分布混杂、10/12 场景形成红线式假两难。整改版 `character-samples-46fe21e7129ee5cf` 已实现 source-first fail-closed validator、内部/玩家双 renderer、四名样板重选、全技能 E/I 交叉报告、双边代价场景及重复度 warning；固定提交 `982e93b` 的 R1 独立复审进一步发现合法但未绑定的 source library 仍可触发构造异常，结论为 `P0=0 / P1=1 / P2=0 / P3=0 / REVIEW_FAIL`。该路径在固定提交 `0831f91` 增加固定 12 人来源预检与两个合法错源回归测试，并由另一名全新 Codex reviewer 完成 R2 终审：`P0=0 / P1=0 / P2=0 / P3=0 / REVIEW_PASS`。此通过只关闭代码与静态产物审查；A1 仍为 `NOT_STARTED`，完整 M12 与 E01–E06 均为 `NOT_RUN`，文化命名包仍为 draft，Gate 1H 人物记忆度证据与 MBTI 玩家验证未完成，PartyValidation、人口状态机及 NPC 完整—简化往返等阶段 C 合同未实现。候选库状态保持 `CANDIDATE_NOT_FROZEN`，不代表阶段 A1/B/C 完成，也不接入 Gate 1 RC。
