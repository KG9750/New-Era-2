...
- D0-A 的开发前提已 `READY`：双轨基线为 `2550617be5eff4226e9e140be741d619fcb6f5a3`；独立 agent `/root/gate1a_test_ops_lead` 担任当前测试运营负责人，Gate 1A 汇总为 #9。
...
- Gate 1A 当前为 `RUNNING_0_OF_7_VALID_A37_PENDING`：A30–A36 七个席位已创建；A30 因同一 host 出现时间重叠的 A30/M-C 会话且受委派 agent 只确认操作 M-C，最终裁定为 `INVALID_TECHNICAL_CONCURRENT_SESSION_CONTAMINATION`。两组三件套永久保留，未授权 M-C 隔离且不计入任何分母，A37 为首个替补；A31 须在逐场隔离预检后启动。
