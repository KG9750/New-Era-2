...
| post-seal correction 基线 | `6203e1c67013b95ee102cf4f8b6c319eb2aa5345` |
| post-seal 聚合审计 | `/root / 2026-07-26T18:52:21Z` |
...
最终分母固定为 A31–A37，共 `7/7` 个有效样本。Post-seal provenance 对账
确认正式 A30 与 M-C 来自两个不同 agent，A30 capture 可归属；但两个会话在
同一 host 重叠 `119,458 ms`，仍违反冻结的逐场串行隔离合同，因此 A30 继续
技术无效。原始证据和历史裁定永久保留，不进入阈值计算，也不把技术污染记作
产品失败。
...
| Week 2 有效编辑 | `2,1,1,2,2,1,1`，中位数 `1` | 中位数 3–5 | **FAIL** |
...
| 未解决 P0 | 0 | 0 | PASS |
| 同根因独立 P1 | 两个聚类各 1 项聚合 P1 finding | 每个聚类 <3 个独立 P1 | PASS |
...
运营依据换周动作顺序和第 8 日目标格执行 override。Post-seal 聚合复核还发现
A31–A37 的 `RESOLVE_LIN_HE_REQUEST` 均改变第 8 日人物活动、产生可见供需与
人物后果并进入确认计划，按玩家测试协议 §8.1 应计为有效编辑，却被机器候选
分类和早期人工表遗漏。纳入后两个周中位数分别为 2 和 1。

## 5. 逐样本代理指标

| 样本 | W1 编辑 | W2 编辑 | 三问题 | 预测 | 人物 | 地图进入 | 地图动作 | W2 完整检查/重排 |
|---|---:|---:|---:|---|---|---|---|---|
| A31 | 3 | 2 | 3 | 是 | 是 | 是 | 是 | 否 / 否 |
| A32 | 1 | 1 | 3 | 是 | 是 | 是 | 否 | 否 / 否 |
| A33 | 1 | 1 | 3 | 是 | 是 | 是 | 否 | 否 / 否 |
| A34 | 2 | 2 | 3 | 是 | 是 | 是 | 是 | 否 / 否 |
| A35 | 3 | 2 | 3 | 是 | 是 | 是 | 是 | 否 / 否 |
| A36 | 2 | 1 | 3 | 是 | 是 | 是 | 是 | 否 / 否 |
| A37 | 2 | 1 | 3 | 是 | 是 | 是 | 是 | 否 / 否 |
...
| 根因聚类 | finding / 严重度 | 有效独立样本 | 影响 |
|---|---|---|---|
| `REQUEST_ACTION_CANDIDATE_CLASSIFICATION` | request action omission / P1 | A31–A37（7/7） | `RESOLVE_LIN_HE_REQUEST` 满足 §8.1，却未进入机器候选与早期人工计数，导致 W2 汇总错误 |
| `WEEK_BOUNDARY_PHASE_AT_TICK_1002` | week attribution / P1 | A31、A34、A35（3/7） | tick 1002 换周后的第二周日程编辑被机器记入第一周，需要运营 override |
| `WEEK_BOUNDARY_PHASE_AT_TICK_1002` | systematic prestart label / P2 | A32、A33、A34、A36、A37（5/7） | 第二周页面已切换但暂停时钟仍显示周日 23:00；A32 来源为 verbatim，其余来自结构化症状 |

这里的 `P1=2 / P2=1` 是三个聚合 finding 的严重度计数，不把每个受影响样本
重复计为独立 finding。两个根因聚类各只包含一个 P1 finding，因此不触发
“同一根因包含 3 个或以上独立 P1”的失败条件；但两个 P1 都必须在新 RC 前
修复或形成明确处置。
...
- 无 P0；存在两个记录/分类层 P1 和一个系统性 P2，但计划—预测—事件—复盘
  因果仍成立。

反对且具有决定性的证据：

- Week 1 与 Week 2 有效编辑中位数均低于冻结目标；
- 第二周中位数为 1；七个 agent 的人物请求均应计为有效编辑，但仍没有任何
  样本达到目标要求的 3–5 次有效计划编辑；
...
## 9. 外部 pending corrections

本 post-seal correction 不执行 GitHub 写操作，以下外部记录仍待另行追加更正
评论：

- #9：现有评论中的 Week 2 数组/中位数及 P1/P2 汇总仍是旧值；正确值为
  `2,1,1,2,2,1,1`、中位数 `1`、`P0=0 / P1=2 / P2=1`；
- #39：现有评论引用了错误 SHA；A37 正确证据 SHA 为
  `7d85df7fbf4b29f40ccf28127fd27fdca2b468fe`。

以上外部笔误不改变七场证据、样本有效性或
`AGENT_PROXY_FAIL / Gate 1H PENDING / Gate 2 LOCKED`。
