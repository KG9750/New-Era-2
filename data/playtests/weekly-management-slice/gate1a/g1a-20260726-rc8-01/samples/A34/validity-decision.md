...
动作。机器原始候选计数保持 `3/0`；最终人工有效编辑为：

- Week 1：`action-0001`、`action-0002`，共 `2`；
- Week 2：`action-0006`、`action-0008`，共 `2`。

`action-0006 / RESOLVE_LIN_HE_REQUEST` 应计为有效编辑，归入
`REQUEST_ACTION_CANDIDATE_CLASSIFICATION` 的 request omission P1。
`action-0008` 的错周归属与既有 prestart 时钟症状共同归入
`WEEK_BOUNDARY_PHASE_AT_TICK_1002`，分别为 week-attribution P1 与
systematic prestart-label P2。历史字段、`4/7` 快照和原 adjudicator 保留；
`VALID` 不变。
