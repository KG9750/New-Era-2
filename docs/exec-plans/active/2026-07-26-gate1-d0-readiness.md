...
| Gate 1 正式 RC | 完成 | `refs/heads/codex/gate1-rc-20260726.7` 固定指向 `03973fcfc0c244555e7e4a4c623eec3fb8b8e032`；双 clean clone 得到相同 Vite 8 artifact、初态和确定性 archive |
| Gate 1A cohort manifest | 冻结待独立复核 | `g1a-20260726-rc7-01` 固定 A23–A29、显式 capture dir、玩家包 V5、访谈 V3，以及包含终局状态转换故障的 blocked 三件套规则 |
...
1. 由独立 reviewer 对远端 ref、manifest、archive、终局 blocked capture 合同与 Gate 锁给出 `RC_FREEZE=YES`；
2. 关闭 #7，创建 A23–A29 七个席位并在 #9 记录新 roster；
3. 启动 manifest 冻结的 4196 loopback capture host并完成空白会话预检；
4. 按 A23–A25、A26–A28、A29 三批逐场执行；每场先保存下载、核验三件套、清空，再发送统一结束访谈；
